...
export const addNewTask = createAsyncThunk('task/addNewTask', async (taskData, {rejectWithValue}) => {
...
export const getAllTasks = createAsyncThunk('task/getAllTasks', async ( queryParams = {}, {rejectWithValue}) => {
    const token = localStorage.getItem('accessToken')
    if(!token){
    ...
    try{        
        ...
        const response = await axios.get(`http://localhost:3000/tasks?${params.toString()}`, {
            headers : {
...
export const updateTask = createAsyncThunk('/task/updateTask', async ({id, updatedData}, {rejectWithValue}) => {
    const token = localStorage.getItem('accessToken')
    if(!token){
        return rejectWithValue("No token found")
    }
    try{
        const response = await axios.post(`http://localhost:3000/tasks/${id}`, updatedData, {
            headers : {"Authorization" : `Bearer ${token}`}
        })        
...
const tasksSlice = createSlice({
    ...
    extraReducers : (builder) => {
        ...
        .addCase(deleteTask.fulfilled, (state, action) => {
            state.status = 'success'
            const taskIndex = state.tasks.findIndex((task) => task._id === action.payload.deletedTask._id)
            if(taskIndex){
